<template>
    <div class="content">
        <template v-for="(item, index) in fieldList">
            <div v-show="!foldBtnHidden && icon === 'close' ? index < count : true" :key="index">
                <div class="item-style">
                    ...
                    ><template slot="prepend">{{ item.label }}</template></el-input>
                    <!-- 日期/时间 -->
                    <el-date-picker
                            v-if="item.type === 'date'"
        ...
        <div v-if="!btnHidden && fieldList.length !== 1" class="btn-style">
            <el-button
        ...
        >{{ icon === 'open' ? '收起' : '展开' }}
            <i :class="icon === 'open' ? 'el-icon-caret-top' : 'el-icon-caret-bottom'"></i>
        </el-button>
    </div>
</template>
<script>
